...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
    ...
    @Query("SELECT u FROM Usuario u WHERE u.grupo.id = :grupoId AND u.rol.nombreRol = :nombreRol")
    List<Usuario> findByGrupoIdAndRolNombreRol(@Param("grupoId") Long grupoId, @Param("nombreRol") RolUsuario nombreRol);

    @Query("SELECT u FROM Usuario u WHERE u.grupo.id = :grupoId")
    List<Usuario> findAllByGrupoId(@Param("grupoId") Long grupoId);
}
